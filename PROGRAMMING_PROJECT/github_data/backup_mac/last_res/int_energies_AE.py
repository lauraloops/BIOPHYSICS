...
# We use the "Non-polar ABS" column (index 8) to match your earlier script output
# where EXP_NACCESS = 89.8 for GLN A 42 (from the shown .rsa line).
# ---------------------------
def parse_naccess_rsa(path):
    """
    Returns dict:
      ASA[(chain_id, resseq, icode)] = asa_value   (float)

    We take Non-polar ABS (Å^2) from .rsa:
      RES <name> <chain> <num>  All ABS All REL Side ABS Side REL Main ABS Main REL NonPolar ABS NonPolar REL Polar ABS Polar REL
    """
    asa = {}
    with open(path, "r") as f:
        for line in f:
            ...
            # split numeric part + optional insertion code
            num_str = ""
            icode = " "
            ...
            # Non-polar ABS is at index 8 in your .rsa example
            try:
                asa_val = float(parts[8])
            except ValueError:
                continue
...
def load_annotated_structure(pdb_file, pdbqt_file, vdw_file,
    ...
    # Attach residue ASA:
    #   res.xtra["ASA_BOUND"]   from complex RSA
    #   res.xtra["ASA_UNBOUND"] from chain-specific RSA
    missing_bound = 0
    missing_unb = 0
    total_res = 0
    ...
    for chain in model:
        for res in chain:
            ...
            asa_b = asa_complex.get(key, None)
            if asa_b is None:
                asa_b = asa_complex.get((chain.id, resseq, " "), None)
            if asa_b is None:
                missing_bound += 1
                asa_b = 0.0
            ...
            if chain.id == "A":
                asa_u = asa_A_unb.get(key, None)
                if asa_u is None:
                    asa_u = asa_A_unb.get((chain.id, resseq, " "), None)
            elif chain.id == "E":
                asa_u = asa_E_unb.get(key, None)
                if asa_u is None:
                    asa_u = asa_E_unb.get((chain.id, resseq, " "), None)
            else:
                asa_u = 0.0
...
def lennard_jones_energy(atom_i, atom_j):
    """12-6 Lennard-Jones with combining rules, kcal/mol"""
...
def residue_pair_energy(resA, resE, cutoff=8.0):
    """Sum electrostatic + vdW between residues across chains within cutoff (Å)."""
    E_elec = 0.0
    E_vdw = 0.0
    for atA in resA:
...
# E_solv(res) = (ASA_res / N_atoms) * sum_i fsrf(atom_i)
#
# We compute:
#   ΔE_solv(res) = E_solv_bound(res) - E_solv_unbound(res)
# using ASA_BOUND from complex RSA and ASA_UNBOUND from chain RSA
#
# Units:
#   ASA in Å^2, fsrf in (kcal/mol)/Å^2  -> energy in kcal/mol
# ---------------------------
...
# ---------------------------
def main():
    # Files (adapt if needed)
    pdb_file = "6m0j_prepared.pdb"
    pdbqt_file = "6m0j_fixed2.pdbqt"
    vdw_file = os.path.join("data", "vdwprm")

    rsa_complex = os.path.join("naccess_res", "6m0j_prepared.rsa")
    rsa_A = os.path.join("naccess_res", "6m0j_chain_A.rsa")
    rsa_E = os.path.join("naccess_res", "6m0j_chain_E.rsa")
    ...
    # Sanity check: should be non-zero when using .rsa
    print(f"Sanity check ASA_BOUND A353 (Å^2): {model['A'][353].xtra.get('ASA_BOUND', 0.0):.2f}")

    # Interface residues based on cutoff_contact
    interface_A = set()
    interface_E = set()
    ...
    # Per-residue interaction energies (A residues vs E interface)
    res_energy_A = {}
    res_energy_E = {}

    # ---- Chain A per-residue: elec/vdw cross terms + delta solvation (bound - unbound)
    for resA in chainA:
        ...
        dSolv = delta_solvation_residue(resA)  # kcal/mol
    ...
    # Totals (IMPORTANT: do NOT sum both A and E elec/vdw, it would double count)
    tot_elec = sum(v[0] for v in res_energy_A.values())
    tot_vdw  = sum(v[1] for v in res_energy_A.values())

    # For solvation change, summing A + E is correct (solv is per-chain term, not a pairwise cross-term)
    ...
    # Export CSV (per residue)
    out_csv = "interaction_energies_RBD_ACE2.csv"
    with open(out_csv, "w", newline="") as f:
        ...
        w.writerow([])
        w.writerow(["TOTAL(A–E)", "",
                    tot_elec, tot_vdw, tot_dsolv, tot_dG])
