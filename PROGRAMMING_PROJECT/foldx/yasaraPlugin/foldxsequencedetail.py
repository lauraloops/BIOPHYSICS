...
def WriteConfigFile(cachedir):
  optionsfile = open(os.path.join(cachedir,"config_SD.cfg"),"w")
  optionsfile.close()
  AddOption(cachedir,"command=SequenceDetail")
...
def AddOption(cachedir,option):
  optionsfile = open(os.path.join(cachedir,"config_SD.cfg"),"a")
  optionsfile.write(option+"\n")
  optionsfile.close()

# SequenceDetail
# ==============
def SequenceDetail_Object(cachedir,foldxbin,objectyas):
  # set pdb filename
  ...
  yasara.ShowMessage("Running FoldX SequenceDetail on Object "+str(objectyas.number.inyas)+". "+citation)
  # make command file
  WriteConfigFile(cachedir)
  # run stability for selected object
  AddOption(cachedir,"pdb="+pdbfilename)
  # build the FoldX command
  foldxcommand = foldxbin+ " -f config_SD.cfg"
  # run SequenceDetail
