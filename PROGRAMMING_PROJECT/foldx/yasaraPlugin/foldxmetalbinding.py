...
# Write metalbinding command file
# ============================
def WriteConfigFile(cachedir):
  optionsfile = open(os.path.join(cachedir,"config_MB.cfg"),"w")
  optionsfile.close()
  AddOption(cachedir,"command=MetalBinding")
  AddOption(cachedir,"metal=-PREDICT")
  AddOption(cachedir,"noHeader=true")
  AddOption(cachedir,"overwriteBatch=false")

# Write metalbinding options file
# ============================
def AddOption(cachedir,option):
  optionsfile = open(os.path.join(cachedir,"config_MB.cfg"),"a")
  optionsfile.write(option+"\n")
  optionsfile.close()

# MetalBinding
# ==============
def MetalBinding_Object(cachedir,foldxbin,objectyas,metal):
  # set pdb filename
  ...
  yasara.ShowMessage("Running FoldX Metal Binding prediction on Object "+str(objectyas.number.inyas)+". "+citation)
  # make command file
  WriteConfigFile(cachedir)
  # run metalbinding for selected object
  AddOption(cachedir,"pdb="+pdbfilename)
  AddOption(cachedir,"metal_element="+metal)
  # build the FoldX command
  foldxcommand = foldxbin+ " -f config_MB.cfg"
  # run Stability
